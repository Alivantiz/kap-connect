...
import { IconBack, IconStar, IconTelegram, IconLogout, IconClose } from '../components/Icons'
    ...
    <>
      <div className="profile-header">
        ...
        <div className="profile-row">
          ...
          <div style={{ flex: 1 }}>
            ...
            {profile.position && <div className="profile-role">{profile.position}</div>}
            <div className="profile-dzo">
              {[profile.dzo, profile.region].filter(Boolean).join(' · ')}
            </div>
            {profile.is_expert && (
              <div className="expert-pill">
                <IconStar size={12} active /> Эксперт {profile.specialty ? '· ' + profile.specialty : ''}
              </div>
            )}
          </div>
        </div>
        ...
        <div className="profile-stats">
          ...
          <div className="pstat">
            <div className="pstat-val">{profile.experience_years || 0}</div>
            <div className="pstat-key">лет стажа</div>
          </div>
          <div className="pstat">
            <div className="pstat-val">{profile.specialty ? profile.specialty.slice(0, 6) : '—'}</div>
            <div className="pstat-key">профиль</div>
          </div>
        </div>
      </div>
      ...
      {profile.skills?.length > 0 && (
        <div className="profile-section">
          <div className="ps-label">Специализация</div>
      ...
      {profile.equipment?.length > 0 && (
        <div className="profile-section">
          ...
          <div className="skill-wrap">
            {profile.equipment.map(s => <span className="skill" key={s}>{s}</span>)}
          </div>
        </div>
      )}
      ...
      {!isMe && profile.telegram && (
        <a className="btn-primary" style={{textDecoration:'none', display:'flex', alignItems:'center', justifyContent:'center', gap:8}}
           href={`https://t.me/${profile.telegram.replace('@','')}`} target="_blank" rel="noreferrer">
      ...
      {isMe && (
        <>
          <button className="btn-primary" onClick={() => setEditing(true)}>Редактировать профиль</button>
...
function EditProfile({ profile, onClose, onSaved }) {
  ...
  const save = async () => {
    ...
    const { data } = await supabase.from('profiles')
      .update(upd).eq('id', profile.id).select().single()
    setSaving(false)
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="field"><label>Имя и фамилия</label>
          <input value={f.full_name} onChange={set('full_name')} /></div>
        <div className="field"><label>Должность</label>
          <input value={f.position} onChange={set('position')} placeholder="Слесарь КИПиА 5-го разряда" /></div>
        <div className="field"><label>ДЗО</label>
        ...
        <div className="field"><label>Регион</label>
          <input value={f.region} onChange={set('region')} placeholder="Туркестанская обл." /></div>
        <div className="field"><label>Специальность</label>
          <input value={f.specialty} onChange={set('specialty')} placeholder="КИПиА" /></div>
        <div className="field"><label>Стаж (лет)</label>
          <input type="number" value={f.experience_years} onChange={set('experience_years')} /></div>
        <div className="field"><label>Навыки (через запятую)</label>
          <input value={f.skills} onChange={set('skills')} placeholder="TIA Portal, Profibus, WinCC" /></div>
        <div className="field"><label>Оборудование (через запятую)</label>
          <input value={f.equipment} onChange={set('equipment')} placeholder="Siemens S7-300, Burkert 8694" /></div>
        <div className="field"><label>Telegram (без @)</label>
          <input value={f.telegram} onChange={set('telegram')} placeholder="username" /></div>
        <div className="field"><label>О себе</label>
          <textarea value={f.bio} onChange={set('bio')} placeholder="Кратко о вашем опыте..." /></div>

        <button className="btn-primary" style={{margin:'4px 0 0', width:'100%'}} disabled={saving} onClick={save}>
